...
| 3 | `gate_backend_egress.py` | The **backend/collectors egress contract**: `httpx` is the only permitted client, only in the 8 sanctioned (key/config-gated) modules; `requests`/`urllib`/`aiohttp`/raw sockets anywhere else fail; collectors may use `requests` only in `common/shipper.py`. | `python scripts/gate_backend_egress.py` |
| 4 | `gate_backend_no_config_egress.py` | The runtime half: with a fresh DB and env keys cleared, the routine **background flows** (run create → ingest → complete → detail → sample upload → sandbox demo detonation) make **zero httpx calls** — the httpx client itself is patched to record and block any non-loopback URL. A negative control (dummy AbuseIPDB key + force-refresh) MUST make the probe observe `api.abuseipdb.com`; and a third phase proves **the enrichment cache keeps egress rare** — after that one refresh, repeated reads of the same run are silent (TTL cache hit). | `python scripts/gate_backend_no_config_egress.py` |
