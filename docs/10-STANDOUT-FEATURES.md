...
### 1. IOC Extraction & Export — SHIPPED
...
### 2. Cross-Run IOC Search ("have I seen this before?") — SHIPPED
...
### 3. At-a-Glance Verdict Score — SHIPPED
...
### 4. Screenshot Capture During Detonation — SHIPPED

**What it does:** Periodic screenshots of the screen during a bounded analysis session — catches ransomware notes, fake alert popups, or any GUI element that pure telemetry (process/network events) completely misses. Also a strong, visual "wow" moment in a demo.

**Status:** Implemented in `app/services/screenshots.py`. During a dynamic detonation (`POST /sandbox/detonate/dynamic`), a `ScreenshotSession` background thread captures frames on an interval (`OUTPOST_SCREENSHOT_INTERVAL`, default 10s) by invoking your capture command (`OUTPOST_SCREENSHOT_CMD`, a shell-quoted template with an `{output}` placeholder — e.g. `VBoxManage controlvm <vm> screenshotpng {output}`, `grim`, or `scrot`). Frames land under `OUTPOST_ARTIFACTS_DIR/<run_id>/shot_NNNN.png` with a `manifest.json`; served via `GET /sandbox/detonate/{run_id}/screenshots[/filename]` (strict filename allowlist + PNG magic check) and rendered as a gallery on the sandbox run detail page. Unconfigured → honest "not configured" status, detonation proceeds normally.

---

## Tier 2 — Good value, moderate effort

### 5. Run Comparison / Diff — SHIPPED
...
### 6. Personal IOC Watchlist — SHIPPED
...
### 7. Per-Run Analyst Notes — SHIPPED
...
### 8. Auto-Generated Suricata/Sigma Rule from Findings — SHIPPED
...
### 9. Fuzzy-Hash Family Clustering (no ML required) — SHIPPED
