...
describe("P2 analyst-surface coherence", () => {
  ...
  // --- P2.1: investigation refs + analysis detail investigation link ---
  ...
  const invDetail = {
    ...
    ref_count: 3,
    tags: ["c2", "beaconing"],
    findings: [{ id: 41, run_id: "run1", rule_id: "rule-beacon", rule_name: "beaconing", severity: "malicious", status: "open", details: "beaconing to 203.0.113.88", triggered_at: "2026-08-18T08:00:00Z", investigation_id: "inv-c2" }],
    refs: [
      { ref_type: "run", ref_id: "run1", added_at: "2026-08-18T08:00:00Z" },
      { ref_type: "host", ref_id: HOST_ID, added_at: "2026-08-18T08:01:00Z" },
      { ref_type: "ioc", ref_id: "203.0.113.88", added_at: "2026-08-18T08:02:00Z" },
    ],
    notes: [],
  };
  ...
  function invDetailStub(overrides: Record<string, unknown> = {}) {
    return vi.fn((input: RequestInfo | URL) => {
      const u = String(input);
      if (u.includes("/auth/me"))
        return Promise.resolve(ok({ enabled: true, authenticated: true, role: "analyst", read_only: false, credential_mode: "hash", expires_at: null }));
      ...
      if (u.includes("/platform")) return Promise.resolve(ok({ os: "linux", name: "Linux", release: "6.7", machine: "x86_64", python: "3.12", collector: "auditd" }));
      if (u.includes("/health")) return Promise.resolve(ok(true));
      if (u.includes("/investigations/inv-c2")) return Promise.resolve(ok(overrides.inv ?? invDetail));
      if (u.includes("/investigations")) return Promise.resolve(ok({ total: 1, limit: 50, offset: 0, investigations: [{ id: "inv-c2", title: "C2 beaconing across agent fleet" }] }));
      if (u.includes("/analysis/run1/findings")) return Promise.resolve(ok(overrides.findings ?? [analysisFinding]));
      if (u.includes("/analysis/run1/observations")) return Promise.resolve(ok([]));
      if (u.includes("/analysis/run1")) return Promise.resolve(ok(overrides.job ?? analysisJob));
      if (u.includes("/analysis")) return Promise.resolve(ok([]));
      if (u.includes("/alerts")) return Promise.resolve(ok([]));
      if (u.includes("/runs/")) return Promise.resolve(ok(runDetail));
      return Promise.resolve(ok(overrides.fallback ?? {}));
    });
  }

  it("investigation detail: run ref deep-links to run detail", async () => {
    await renderAt("/investigations/inv-c2", invDetailStub());
    await waitFor(() => expect(linkHrefs()).toContain("/runs/run1"), { timeout: ROUTED_TIMEOUT });
  });

  it("investigation detail: host ref deep-links to the host workspace", async () => {
    await renderAt("/investigations/inv-c2", invDetailStub());
    await waitFor(() => expect(linkHrefs()).toContain(`/hosts/${encodeURIComponent(HOST_ID)}`), { timeout: ROUTED_TIMEOUT });
  });

  it("investigation detail: IOC ref deep-links to the pre-filled search", async () => {
    await renderAt("/investigations/inv-c2", invDetailStub());
    await waitFor(() => expect(linkHrefs().some((h) => h.includes("/search?q=203.0.113.88"))).toBe(true), { timeout: ROUTED_TIMEOUT });
  });

  it("analysis detail: investigation chip links to the investigation workspace", async () => {
    const stub = invDetailStub();
    await renderAt("/analysis/run1", stub);
    // Wait for the findings section to render
    await waitFor(() => {
      const text = document.body.textContent ?? "";
      if (!text.includes("beaconing")) throw new Error(`no beaconing yet: ${text.substring(0, 200)}`);
    }, { timeout: ROUTED_TIMEOUT });
    // Now the investigation chip should be a link
    await waitFor(() => {
      expect(linkHrefs()).toContain("/investigations/inv-c2");
    }, { timeout: ROUTED_TIMEOUT });
  });
});
