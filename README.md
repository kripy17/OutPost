...
| ⌨️ **Terminal mirror** | The `outpost` CLI reaches the same API — **22 commands**, Rich tables, colorized risk, recon markers, rule knobs, alert-queue mirror (`outpost alerts --provenance real|synthetic`, saved per tab via `--save`, wiped with `outpost settings clear-prefs`) |
