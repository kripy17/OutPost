...
| CLI pytest | **127** | rendering regressions (incl. intel-age staleness label), campaigns output, risk columns, YARA + footprint mirrors + exports, rules knobs, agent install (token embed + Windows bats), module entry, rotate-agent-token, fleet auth context in status, admin backfill-channels + pg-migrate wiring, alert-queue mirror (`outpost alerts` with status + provenance filters, `--save` per-tab persistence), `outpost settings clear-prefs` + the prefs store, alert-triage lifecycle (`outpost triage` ack→resolve→reopen with the transition-comment contract, PATCH URL/payload + DELETE parity), `outpost allowlist` group (add/list/remove with the relaxed 200/204 DELETE contract), `outpost rules suppressions` (list/add/remove, run/value/global scopes), **P0.8 client contracts** (`outpost hosts timeline` + `search --global` + analysis-job surfaces with parity tests), **P1.6 CLI workflow completion** (`outpost investigations` group: list/get/create/patch/refs/notes/close/reopen + finding attach/detach carrying status, `outpost analysis` group: launch/list/get/cancel/observations/findings with honest isolated-outpost 501, endpoint/payload parity tests against the P0.8 web contracts) |
| Frontend | **355** | vitest unit tests (page-contract suites for the coverage matrix, footprint topology + registration timeline + WHOIS timeline + breach note + shared-infra clusters + cluster-bar scaling + member-breakdown tooltip + deck-wide fill-pattern language (bars · timeline · kill chain · donut · severity dots) + synthetic detonation scenarios + clipboard fallback + SSE stream-hub fan-out + useEventStream ref pattern + theme-token reads, campaign sorts, sample vault, agents fleet, IOC search, watchlist, findings triage, chart + triage components, run-detail recon/attribution, log-pattern drafts, monitor reconciliation, static-analysis derivations, audit action chips, history archive totals, shared real-first preference (archive ↔ queue), queue-preferences wipe contract, sweep saved-split chips (per-tab derivation + shared vocabulary), one-click client-state reset (provenance + search/YARA/enum/log drafts) reachable from the ⌘K palette, browser-baseline check, **P1.1 investigations** (api.ts contract suite + routed workspace render: list/detail with findings/refs/notes, status pills, close-with-conclusion flow, detach-never-moves-triage contract), **P1.2 analysis workflow** (api.ts observations/findings contracts, routed workspace: status/progress/observations (strings · iocs · pe/elf) + findings attach, cancel → terminal state, honest failed-job error, launch-form POST + navigation, realtime run-update SSE frame driving queued → completed through the shared stream hub), **P1.3 global search UX** (api.ts /search contract, routed page: IOC-scope regression (deep-link ?q= preserved), global-scope grouped results with per-group totals + qualifier echo, honest empty state, per-group deep-links incl. analysis_job → analysis workspace, mode toggle persistence), **P1.4 host investigation** (api.ts /hosts/{id}/timeline contract, routed workspace: host context strip (platform · heartbeat), kind tabs + event-type/free-text filters, merged feed with per-kind chips, deep-links (finding/event → run, analysis_job → analysis, ioc → IOC search), honest empty feed for known-but-quiet hosts, unknown-host 404 state, load-more pagination, hosts deep-link now → /hosts/{id}), **P1.5 realtime analyst surfaces** (fake-EventSource suites pinning live invalidation of the findings queue on alert frames + finding-link run-updates, the investigations list on lifecycle frames, and the investigation workspace on frames naming its own case — with the negative case: a frame naming a DIFFERENT investigation never refetches the open workspace)), **P2 analyst-surface coherence** (host-id deep-links on the findings queue / events feed / run-detail header / history filter chip all lead into the /hosts/{id} aggregate-timeline workspace; attached findings surface their case link on the queue; no case → no link; routed-test waitFor timeouts hardened to the 5000ms convention so the suite is deterministic under a loaded parallel pool)) + clean `tsc --noEmit` + Vite build |
