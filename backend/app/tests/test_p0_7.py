...
def test_analysis_create_emits_job_frame(client, conn):
    ...
    static_frame = captured[0]
    assert static_frame["job_status"] == "completed"
    assert static_frame["progress"] == 100
    assert static_frame["completed"] is True
    # Queued rows still exist (store-created — POST refuses unexecuted
    # backends) and are observable via the API.
    run_id = make_run(client, sample_name="p07-host.bin", platform="linux")
    jobs_store.create_job(conn, run_id, "watched-host")
...
def test_cancel_emits_terminal_canceled_once(client, conn):
    from ..models import analysis_jobs as jobs_store
    from ..services import events_stream

    run_id = make_run(client, sample_name="p07-cancel.bin", platform="linux")
    jobs_store.create_job(conn, run_id, "watched-host")
    conn.commit()  # the API reads through its own connection
...
def test_reconnect_never_replays_frames(client, conn):
    ...
    run_id = make_run(client, sample_name="p07-reconnect.bin", platform="linux")
    jobs_store.create_job(conn, run_id, "watched-host")
    conn.commit()  # the API reads through its own connection
...
def test_job_progress_persisted_and_observable(client, conn):
    ...
    run_id = make_run(client, sample_name="p07-progress.bin", platform="linux")
    job = jobs_store.create_job(conn, run_id, "watched-host")
    conn.commit()  # the API reads through its own connection
