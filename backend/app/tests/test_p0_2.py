...
- Analysis job API: persisted static jobs (real static analysis with stored
  bytes), queued watched-host/external-provider state, cancel transitions,
  isolated-outpost 501, observations (NO observations table), findings.
...
def test_ioc_detail_payload_derives_runs_and_hosts(client, conn):
    ...
    # Wire provenance + finding links directly (P0.2 does not backfill).
    conn.execute("INSERT INTO ioc_provenance (ioc_id, ref_type, ref_id, first_seen) VALUES (?, 'event', (SELECT id FROM events WHERE run_id = ? LIMIT 1), '2026-08-17T00:00:00Z')", (ioc["ioc_id"], run_id))
    conn.execute("INSERT INTO ioc_findings (ioc_id, finding_id) VALUES (?, ?)", (ioc["ioc_id"], aid))
    conn.commit()

    det = client.get(f"/iocs/{ioc['ioc_id']}").json()
    assert det["value"] == "203.0.113.166"
    assert det["provenance"] and det["provenance"][0]["ref_type"] == "event"
    # Finding rides along with the full alert row.
    assert any(f["id"] == aid for f in det["findings"])
    # Runs + hosts DERIVED from the event provenance — not fabricated.
...
def test_job_state_is_persisted(client, conn):
    ...
    job = client.post("/analysis", json={"backend": "watched-host", "sample_name": "p02-persist.bin"}).json()
...
def test_list_and_filter_jobs(client):
    ...
    w = client.post("/analysis", json={"backend": "watched-host", "sample_name": "p02-list-b.bin"}).json()
    e = client.post("/analysis", json={"backend": "external-provider", "sample_name": "p02-list-c.bin"}).json()
...
def test_cancel_transitions(client):
    q = client.post("/analysis", json={"backend": "watched-host", "sample_name": "p02-cancel.bin"}).json()
...
def test_isolated_outpost_returns_501(client):
    before = client.get("/analysis").json()["total"]
    resp = client.post("/analysis", json={"backend": "isolated-outpost", "sample_name": "x.bin"})
    assert resp.status_code == 501
    assert "no isolated execution" in resp.json()["detail"]
    # No run/job was created.
    assert client.get("/analysis").json()["total"] == before


def test_no_observations_table_created(client, conn):
    client.post("/analysis", json={"backend": "watched-host", "sample_name": "p02-noobs.bin"})
    tables = {r[0] for r in conn.execute("SELECT name FROM sqlite_master WHERE type='table'").fetchall()}
    assert "observations" not in tables


def test_analysis_findings_uses_run_relationship(client):
    job = client.post("/analysis", json={"backend": "external-provider", "sample_name": "p02-aj-find.bin"}).json()
