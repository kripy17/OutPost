...
def evaluate_batch(conn: sqlite3.Connection, run_id: str, events: list[dict]) -> list[Alert]:
    ...
    from .ioc_extraction import record_for_alert
    ...
    def fire(alert: Alert, event: dict | None = None) -> bool:
        ...
        held back. After a clean insert, IOC population runs best-effort
        (P3.1): it enriches the finding with entity/provenance records and
        never alters or loses the detection itself.
        """
        ...
        seen_related.add(key)
        finding_id = insert_alert(conn, alert)
        new_alerts.append(alert)
        record_for_alert(conn, alert, finding_id, event.get("id") if event else None)
        return True
    ...
    for host, kind, value, ev in baseline_svc.check_deviations(
        ...
        if not _alert_exists(conn, run_id, "baseline-anomaly", value):
            fire(baseline_svc.build_alert(run_id, host, kind, value, ev), ev)
