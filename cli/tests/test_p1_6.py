...
- the `outpost analysis` group launches, lists, shows, cancels, and renders
  observations/findings — with isolated-outpost rejected up front (the
...
def test_analysis_launch_rejects_isolated_outpost(monkeypatch):
    """isolated-outpost is a reserved enum — the backend 501s it; the CLI
    refuses to pretend there is an isolated execution backend."""
    monkeypatch.setattr(api_client, "create_analysis_job", lambda *a, **k: {**_JOB, "status": "queued"})
    result = runner.invoke(app, ["analysis", "launch", "isolated-outpost", "--sample-id", "s1"])
    assert result.exit_code == 1
    assert "isolated-outpost" in result.output
    assert "no isolated execution backend" in result.output  # may wrap mid-phrase
